...
typedef enum
{
    INFO_RESET_EN = 0x4,   // RST#外部手动复位输入功能是否开启
    INFO_BOOT_EN = 0x8,    // 系统引导程序 BootLoader 是否开启
    INFO_RST_PIN = 0x10,   // 复位引脚选择：1：PA7；0：PA8
    INFO_LOADER  = 0x20,   // 当前系统是否处于Bootloader 区
    STA_SAFEACC_ACT = 0x30,// 当前系统是否处于安全访问状态，否则RWA属性区域不可访问
